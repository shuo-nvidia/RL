...
def setup(
    ...
    assert (                     
        ...
        or (policy_config["dtensor_cfg"]["context_parallel_size"] == 1)
    ), (
        f"zero_outside_topk=True requires context_parallel_size=1, "
        f"but got context_parallel_size={policy_config['dtensor_cfg']['context_parallel_size']}. "
    )
    # Set random seed
