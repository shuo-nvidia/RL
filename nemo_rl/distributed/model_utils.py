...
class ChunkedDistributedTopkLogits(torch.autograd.Function):
    ...
    def backward(
        ...
        for chunk_idx in range(num_chunks):
            ...
            # 1. extract the current chunk data
            logits = vocab_parallel_logits[:, s0:s1, :].to(dtype=torch.float32)
            log_probs = _compute_distributed_log_softmax(logits, group=tp_group)
            ...
            # 2. filter the top-k of the current rank and convert the indices
            ...
            # 3. key: calculate the global sum(grad_prob * softmax_prob) (need to reduce across TP)
            # 3.1 get the softmax value of the top-k of the current rank
            B_chunk = B
            ...
            # 3.2 calculate the local "gradient×probability" (only the top-k of the current rank contributes)
            local_gp = chunk_grad_probs * topk_softmax_vals * in_range_mask.float()  # [B, chunk_len, k]
            # 3.3 reduce across TP, get the global sum (the sum of each [B, chunk_len] position)
            ...
            # 4. complete gradient formula: p * (g_p - global_sum_gp)
            grad_contributions = topk_softmax_vals * (chunk_grad_probs - global_sum_gp.expand_as(chunk_grad_probs))
            grad_contributions = grad_contributions * in_range_mask.float()  

            # 5. vectorized scatter gradient
            ...
            # 6. turn back to the original dtype
