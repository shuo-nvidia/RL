...
def setup_distillation_test_data(batch_size=2, seq_len=4, vocab_size=8, topk=64):
    ...
    teacher_topk_indices = torch.randint(0, vocab_size, (batch_size, seq_len, topk), device=device)
...
def test_distillation_loss_temperature_scaling():
    """Test temperature scaling effect on loss calculation."""
    data, student_logits = setup_distillation_test_data()
    ...
    assert not all(abs(losses[i] - losses[i+1]) < 1e-6 for i in range(len(losses)-1))
...
def test_distillation_loss_invalid_k_zero():
    ...
    try:
        ...
        assert torch.isnan(loss) or torch.isinf(loss), f"Expected NaN or Inf for k=0, got {loss}"
        
    except (ValueError, RuntimeError) as e:
        # Expected behavior: k=0 should raise an error
        assert "k" in str(e).lower() or "empty" in str(e).lower() or "zero" in str(e).lower()
